...
SubShader
{
    ...
    Pass
    {
        ...
        void geom(triangle v2g IN[3], inout PointStream<g2f> pointStream)
        {
            ...
            fixed2 xy = (IN[0].uv + IN[1].uv + IN[2].uv) / 3;
            
			o.vertex = mul(UNITY_MATRIX_MVP, fixed4(tempPos, 1));
			o.uv = lerp(IN[0].uv, xy, 0.2);
            pointStream.Append(o);

			o.vertex = mul(UNITY_MATRIX_MVP, fixed4(tempPos, 1));
			o.uv = lerp(IN[1].uv, xy, 0.2);
            pointStream.Append(o);

			o.vertex = mul(UNITY_MATRIX_MVP, fixed4(tempPos, 1));
			o.uv = lerp(IN[2].uv, xy, 0.2);
            pointStream.Append(o);
